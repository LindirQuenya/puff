...
Begin  {* Puff_Start *}
  ...
  If puff_file = 'setup' Then
    ...
    (**** Make_Text_Border doesn't work here *****)
      ...
      TextCol(white);
      GotoXY(32,4);
      Write('PUFF, Version 20181104');
      Gotoxy(31,7);
      Write('Copyright (C) 1991');
      GotoXY(15,8);
      Write('Scott W. Wedge, Richard Compton, David Rutledge');
      GotoXY(30,10);
      Write('(C) 1997, 1998 Andreas Gerstlauer');
      GotoXY(8,13);
      Write('2010 - Code released under the GNU General Public License version 3');
      GotoXY(25,16);
      Write('Linux version by:');
      GotoXY(25,18);
      Write('2000-2018 Pieter-Tjerk de Boer');
      GotoXY(25,19);
      Write('     2009 Leland C. Scott');
      GotoXY(2,27);
      Write('For more information, see:');
      GotoXY(2,29);
      Write('http://www.its.caltech.edu/~mmic/puff.html -> for the original version');
      GotoXY(2,31);
      Write('http://wwwhome.cs.utwente.nl/~ptdeboer/ham/puff.html -> for the Linux version');
      TextCol(white);
      message_color := white;
      Blank_Blue_Cursor;     { make cursor invisible }
      Repeat
        Ch := ReadKey
      Until (Ch<>screenresize);
      If (Ch = #0) Then Ch := char(ord(ReadKey)+128);
      If Ch = Esc Then
        Begin       { Has the Esc key been pressed? }
          TextMode(OrigMode);
          ClrScr;
...
{**
  ...
  **}
  ...
  If Not(fileexists(puff_file<>'setup.puf',net_file,puff_file)) Then
    Begin
      If Not(setupexists(puff_file)) Then
        Begin
          erase_message;
          message[2] := 'setup.puf';
          message[3] := 'not found';
...
{ from the excess rows beyond 34, first 1 or 2 are used to expand the BOARD window (to make the entire help message fit); anything left is used to expand the PARTS window }
  ...
  If (i>=36) Then
    ...
    End;
  ymin[2] := 2;
  ymax[2] := 8;                       { plot window }
  ymin[3] := 16;
  ymax[3] := ymin[3]+parts_height-1;  { parts window }
  ymin[4] := ymax[3]+3;
  ymax[4] := ymin[4]+board_height-1;  { board window }
  ymin[5] := ymax[3]+1;
  ymax[5] := ymin[3]+17;              { extra parts window }
  ymin[6] := 11;
  ymax[6] := 13;                      { help window }

  i := 14 * ((ymax[12]+1) Div 28);
  ...
  Repeat
    Begin
      ...
      xmax[9] := xmin[10];
    End
  Until (xmax[9]-xmin[9] >= ymax[9]-ymin[9]);
  ymin[11] := i;
