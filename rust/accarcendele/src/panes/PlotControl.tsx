...
export function PlotControl() {
  const [sToFrom, _setSToFrom] = useState([
    [1, 1],
    [2, 1],
    [3, 1],
    [4, 1],
  ...
  const sToFrom_usable = sToFrom
    .map((v, i) => {
      return { v: v, c: colors[i] };
    })
    .filter(
      (o) =>
        availablePorts.includes((o["v"][0] ?? 10) - 1) &&
        availablePorts.includes((o["v"][1] ?? 10) - 1),
    )
    .map((o) => [o["v"][0], o["v"][1], o["c"]] as [number, number, string]);
  const [freqLim, setFreqLim] = useState(["1k", "5G"]);
  const freqLim_parsed = freqLim
    .map((l) => extract_float(l, 0, false)?.[0])
      ...
      <table
        ...
        onKeyDownCapture={(e) => {
          if (e.key.toLowerCase() === "p") {
            ...
            // Try to plot.
            if (
              sToFrom_usable.length > 0 &&
              freqLim_parsed.length == 2 &&
              nPoints_parsed !== undefined
            ) {
              console.log("PlotControl: do-plot");
              emit("do-plot", {
                params: sToFrom_usable,
                freqLim: freqLim_parsed,
                nPoints: nPoints_parsed,
              } as DoPlotEvent);
            }
          }
        }}
        ...
        <tbody>
          ...
          <tr>
            ...
            <th>
              <input
                value={smithR}
                onChange={(e) => setSmithR(e.target.value)}
              />
            </th>
          </tr>
          <tr>
            ...
            <th>
              <input
                value={freqLim[0]}
                onChange={(e) => setFreqLim([e.target.value, freqLim[1]])}
              />
            </th>
            <td>Hz</td>
          </tr>
          <tr>
            ...
            <th>
              <input
                value={freqLim[1]}
                onChange={(e) => setFreqLim([freqLim[0], e.target.value])}
              />
            </th>
            <td>Hz</td>
          </tr>
