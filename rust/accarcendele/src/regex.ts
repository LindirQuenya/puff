import { Dictionary, Impedance, ImpedanceUnit, Length, LengthUnit } from "./types";

export const TLINE_LABEL = /^t[a-zA-Z]*\s*/;
export const POSITIVE_FLOAT_CHUNK = /^(\+?(?:(?:\d+(?:\.\d*)?)|(?:\.\d+)))\s*([fpnumckMGTP]?)\s*/;
export const IMPEDANCE_UNIT = /^([zZyYsSoOΩ])\s*/;
// TODO: support manhattan-length components?
export const LENGTH_UNIT = /^([mhdD°])\s*/
export const GENERIC_FLOAT_CHUNK = /^([+-]?(?:(?:\d+(?:\.\d*)?)|(?:\.\d+)))\s*([fpnumckMGTP]?)\s*/;
...
export function extract_float(s: string, start: number, allowNegative?: boolean): [number, number] | null {
	const match = s.slice(start).match(allowNegative ? GENERIC_FLOAT_CHUNK : POSITIVE_FLOAT_CHUNK);
	if (!match) {
		return null;
	}
...
export function extract_impedance(s: string, start: number): [Impedance, number] | null {
...
export function extract_length(s: string, start: number, allowNegative?: boolean): [Length, number] | null {
