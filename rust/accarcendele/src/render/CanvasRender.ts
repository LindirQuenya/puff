...
export class CanvasRender implements Render {
  ...
  pos_to_px(pos: PhysicalCoordinates): PixelCoordinates {
    return {
      x_px:
        (pos.x_m * (this.canvas_size.x_px - 2 * this.border.x_px)) /
          this.board_size.x_m +
        this.border.x_px,
      y_px:
        (pos.y_m * (this.canvas_size.y_px - 2 * this.border.y_px)) /
          this.board_size.y_m +
        this.border.y_px,
    };
  }
  ...
  clamp(pos: PixelCoordinates): PixelCoordinates {
    return {
      x_px: Math.max(
        this.border.x_px,
        Math.min(this.canvas_size.x_px - this.border.x_px, pos.x_px),
      ),
      y_px: Math.max(
        this.border.y_px,
        Math.min(this.canvas_size.y_px - this.border.y_px, pos.y_px),
      ),
    };
  }
  ...
  draw_ports(
    ...
    for (let p = 0; p < 4; p++) {
      if (to[p]) {
        ...
        const upperX =
          pos.x_px +
          halfw *
            Math.sign(pos.y_px - port.y_px) *
            Math.sign(pos.x_px - port.x_px);
    ...
    if (this.hasborder()) {
      for (let p = 0; p < 4; p++) {
        ...
        this.ctx.fillText(
          `${p + 1}`,
          pos.x_px - this.border.x_px * (((1 + p) % 2) - (p % 2) / 3),
          pos.y_px + 5,
        );
      }
    }
